...
function init_right_top_tools() {
    try {
        ...
        // On click - image
        $(selector_meme_generator + " .image").off("click");
        $(selector_meme_generator + " .image").on("click", function () {
            try {
                // Image
                fabric.Image.fromURL("/dist/img/memes/assets/1_Fur/Fur_OG.png?v=3").then((image) => {
                    // Set attributes
                    image.set({
                        hasControls: true,
                        hoverCursor: "auto",
